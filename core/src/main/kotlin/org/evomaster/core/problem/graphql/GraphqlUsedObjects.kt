...
class GraphqlUsedObjects {
    /**
     * The variables usage are the same as the variables in the REST problem
     */
    ...
    fun assign(key:Pair<GraphqlCallAction, Gene>, value:Gene, selectedField:Pair<String, String>){
    ...
    fun selectbody(action:GraphqlCallAction, obj:Gene){
    ...
    fun getRelevantGene(action: GraphqlCallAction, gene: Gene): Gene{
    ...
    fun exists(action: GraphqlCallAction): Boolean{
        return mapping.keys.map { it.first }.contains((action as GraphqlCallAction).id)
    }
    fun allExist(actions: MutableList<GraphqlAction>): Boolean {
        val restActions = actions.filter{ it::class == GraphqlCallAction::class}
        return mapping.keys.map { it.first }.containsAll(restActions.map { (it as GraphqlCallAction).id })
    }

    fun notCoveredActions(actions: MutableList<GraphqlAction>): MutableList<GraphqlCallAction> {
        val restActions = actions.filter{ it::class == GraphqlCallAction::class}.map { (it as GraphqlCallAction) }.toMutableList()
    ...
    fun getAllRelevantObjects(action: GraphqlCallAction): List<Gene>{
