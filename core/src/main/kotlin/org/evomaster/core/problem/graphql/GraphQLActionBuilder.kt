...
object GraphQLActionBuilder {
    ...
    private const val maxDepth: Int = 10
    ...
    private fun extractParams(
        ...
        if (tableFieldWithArgs) {
            ...
            val gene = getReturnGene(state, tableFieldType, kindOfTableField, kindOfTableFieldType, tableType, history,
                    isKindOfTableFieldTypeOptional, isKindOfTableFieldOptional, enumValues, methodName, unionTypes, interfaceTypes, accum)
        ...
        when (kindOfTableField?.toLowerCase()) {
            ...
            objectTag -> {
                ...
                return if (checkDepth(accum)) {
                    ...
                    OptionalGene(tableType, CycleObjectGene(tableType))//*/*
                }
            }
            unionTag -> {
                accum += 1
                return if (checkDepth(accum)) {//todo not sure about this accum and check here
                    history.addLast(tableType)
            ...
            "null" ->
                return getReturnGene(state, tableType, kindOfTableFieldType, kindOfTableField, tableFieldType, history, isKindOfTableFieldTypeOptional, isKindOfTableFieldOptional, enumValues, methodName, unionTypes, interfaceTypes, accum)
            "date" ->
    ...
    private fun createUnionObjectsGene(
        ...
        for (elementInUnionTypes in unionTypes) {//Browse all objects defining the union
            ...
            if (checkDepth(accum)) {
                ...
                fields.add(OptionalGene(elementInUnionTypes, CycleObjectGene(elementInUnionTypes)))/**/
            }
