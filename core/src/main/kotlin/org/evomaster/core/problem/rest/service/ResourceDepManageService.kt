...
//            }
    ...
    fun compare(actionName : String, eviA : EvaluatedIndividual<RestIndividual>, eviB : EvaluatedIndividual<RestIndividual>) : Int{
     ...
     */
    fun compare(actionA : Int, eviA : EvaluatedIndividual<RestIndividual>, actionB: Int, eviB : EvaluatedIndividual<RestIndividual>) : Int{
    ...
    fun findDependentResources(ind: RestIndividual, call : RestResourceCalls, minProbability : Double = 0.0, maxProbability : Double = 1.0): MutableList<RestResourceCalls>{
    ...
    fun findNonDependentResources(ind: RestIndividual, call : RestResourceCalls): MutableList<RestResourceCalls>{
        return ind.getResourceCalls().filter { other ->
            (other != call) && uncorrelated[call.getResourceNodeKey()]?.contains(other.getResourceNodeKey())?:false
        }.toMutableList()
    }

    fun existsDependentResources(ind: RestIndividual, call : RestResourceCalls, minProbability : Double = 0.0, maxProbability : Double = 1.0): Boolean{
        return ind.getResourceCalls().find {other ->
            (other != call) && dependencies[call.getResourceNodeKey()]?.find { r->r.targets.contains(other.getResourceNodeKey()!!) && r.probability >= minProbability && r.probability <= maxProbability} !=null
        }!=null
    }

    fun isNonDepResources(ind: RestIndividual, call : RestResourceCalls): Boolean{
        return ind.getResourceCalls().find {other ->
...
//    }
    ...
    private fun handleAddNewDepResource(sequence: MutableList<RestResourceCalls>, maxTestSize : Int) : Pair<RestResourceCalls?, RestResourceCalls>?{
        ...
        if(candidates.isNotEmpty()){
             ...
             */
            dependencies[first.getResourceNodeKey()]!!.flatMap {
