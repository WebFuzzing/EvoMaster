...
class TestSuiteWriter {
    ...
    private fun classDescriptionEmptyLine(lines: Lines) = when {
        ...
        config.outputFormat.isPython() -> lines.add("#")
        else -> lines.add(" * ")
    }
    ...
    private fun classDescriptionComment(solution: Solution<*>, lines: Lines, timestamp: String = "") {
        ...
        classDescriptionEmptyLine(lines)
        lines.add("$comment Used time: ${searchTimeController.getElapsedTime()}")
        classDescriptionEmptyLine(lines)
        lines.add("$comment Needed budget for current results: ${searchTimeController.neededBudget()}")
        classDescriptionEmptyLine(lines)
        lines.add("$comment ${solution.termination.comment}")
        if (!format.isPython())
            lines.add(" */")
    }
    ...
    private fun staticVariables(controllerName: String?, controllerInput: String?, lines: Lines) {
        ...
        if (config.outputFormat.isJava()) {
            ...
            if (!config.blackBox || config.bbExperiments) {
                ...
                lines.add("$baseUrlOfSut = \"${BlackBoxUtils.restUrl(config)}\"")
            }
        }
