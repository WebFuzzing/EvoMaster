...
class TestSuiteWriter {
    ...
    private fun classDescriptionEmptyLine(lines: Lines) {
        if (config.outputFormat.isJava()) {
            ...
            lines.addBlockCommentLine(" ", config.outputFormat)
        }
    }
    ...
    private fun classDescriptionComment(solution: Solution<*>, lines: Lines, timestamp: String = "") {
        ...
        lines.startCommentBlock(format)
        ...
        if (timestamp != "") {
            lines.addBlockCommentLine(" ************************************ WARNING ************************************ ", format)
            lines.addBlockCommentLine(" * This is an snapshot of the generated tests after $timestamp seconds elapsed. *", format)
            lines.addBlockCommentLine(" * The execution of Evomaster has not finished. *", format)
            lines.addBlockCommentLine(" ********************************************************************************* ", format)
        }

        lines.addBlockCommentLine(" The generated test suite contains ${solution.individuals.size} tests", format)
        classDescriptionEmptyLine(lines)
        lines.addBlockCommentLine(" Covered targets: ${solution.overall.coveredTargets()}", format)
        classDescriptionEmptyLine(lines)
        lines.addBlockCommentLine(" Used time: ${searchTimeController.getElapsedTime()}", format)
        classDescriptionEmptyLine(lines)
        lines.addBlockCommentLine(" Needed budget for current results: ${searchTimeController.neededBudget()}", format)
        classDescriptionEmptyLine(lines)
        lines.addBlockCommentLine(" ${solution.termination.comment}", format)
        lines.endCommentBlock(format)
