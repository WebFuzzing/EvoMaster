        ...
        if (this.index == next.index) {
            ...
            if (mutatedGenes.getAdded(true).isNotEmpty()) { //add new action
                ...
                val groupGeneByActionIndex = addedGenes.filter { it.gene != null }.groupBy { g ->
                    mutatedGenes.mutatedIndividual!!.seeActions(NO_INIT).find { a -> a.seeTopGenes().contains(g.gene) }
                        ?.getLocalId()
                }

                //handle added actions without genes
                val emptyActions = addedGenes.filter { it.gene == null }.mapNotNull { it.actionLocalId }

                addedGenes.mapNotNull { it.actionLocalId }.forEach { localId ->
                    val actionOfAddedGene = individual.findActionByLocalId(localId, false)!!
                    if (emptyActions.contains(localId)) {
                        impactInfo!!.addOrUpdateActionGeneImpacts(
                            action = actionOfAddedGene,
                            newAction = true,
                            impacts = mutableMapOf()
                        )
                    } else {
                        val mgenes = groupGeneByActionIndex.getValue(localId)
//                        val index = mgenes.mapNotNull { it.actionLocalId }.toSet()
//                        if (index.size != 1 || index.first() != localId)
//                            throw IllegalArgumentException("mismatched impact info: genes should be mutated at $index action, but actually the index is $actionIndex")
            ...
            if (mutatedGenes.getSwap().isNotEmpty()) {
                ...
                impactInfo!!.swapActionGeneImpact(from.map { individual.getActionIndexByLocalId(it, MAIN_EXECUTABLE, false) }, to.map { individual.getActionIndexByLocalId(it, MAIN_EXECUTABLE, false) })
            }
        ...
        mutatedGenes.mutatedActionOrInit().forEach { isInit ->
            ...
            mutatedGenesWithContext.forEach { gc ->
                val impact = impactInfo!!.getGene(
                    ...
                    actionLocalId = gc.actionLocalId,
    ...
    fun getImpact(individual: Individual, gene: Gene): GeneImpact? {
         ...
         */
        action =
            individual.seeInitializingActions().filterIsInstance<DbAction>().find { it.seeTopGenes().contains(gene) }
    ...
    fun getImpactByAction(actionIndex: Int, fromInitialization: Boolean): MutableMap<String, GeneImpact>? {
        ...
        return impactInfo.findImpactsByAction(
            ...
            actionName = if (fromInitialization) individual.seeInitializingActions()[actionIndex].getName() else individual.seeActions(
