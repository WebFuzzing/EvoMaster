...
class LongGeneValue(
        name: String,
        value: Long = 0
) : NumberGeneValue<Long>(name, value) {

    companion object{
        private val log : Logger = LoggerFactory.getLogger(LongGeneValue::class.java)
    }

    override fun copy(): Gene {
        val copy = LongGeneValue(name, value)
        return copy
    }
    ...
    override fun bindValueBasedOn(gene: Gene): Boolean {
        when(gene){
            is LongGeneValue -> value = gene.value
            is FloatGeneValue -> value = gene.value.toLong()
            is IntegerGeneValue -> value = gene.value.toLong()
            is DoubleGeneValue -> value = gene.value.toLong()
            is StringGene -> {
