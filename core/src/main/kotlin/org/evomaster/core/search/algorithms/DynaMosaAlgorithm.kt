...
/**
 * DynaMOSA variant: identical to MOSA but uses a dynamic, reduced focus set of targets per generation.
 */
class DynaMosaAlgorithm<T> : SearchAlgorithm<T>() where T : Individual {
    ...
    override fun setupBeforeSearch() {
        ...
        // Validate CFG completeness and graph mapping (logs only)
        try {
            goalsManager.validateCfgCompletenessForBranchTargets()
        } catch (_: Throwable) {
            // keep setup robust if validation fails
        }
    }

    override fun searchOnce() {

        // Optional early-stop: nothing left to cover according to goals manager
        if (goalsManager.getUncoveredGoals().isEmpty()) {
            return
        }
