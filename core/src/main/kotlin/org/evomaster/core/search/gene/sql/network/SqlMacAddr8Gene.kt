...
class SqlMacAddr8Gene(
    name: String,
    val octets: List<IntegerGene> = List(MACADDR8_SIZE)
    { i -> IntegerGene("b$i", min = 0, max = 255) }
) : Gene(name, octets.toMutableList()) {

    companion object {
        val MACADDR8_SIZE = 8
        val log: Logger = LoggerFactory.getLogger(SqlMacAddr8Gene::class.java)
    }
