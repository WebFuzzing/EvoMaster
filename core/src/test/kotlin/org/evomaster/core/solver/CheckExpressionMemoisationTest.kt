...
class CheckExpressionMemoisationTest {
    ...
    /** The shape an ORM emits for an enum column on PostgreSQL. Wide enough for its parse to be
     *  measurable: real schemas carry these with hundreds of values. */
    private fun enumCheck(seed: Int, values: Int = 200): String {
        val array = (1..values).joinToString(", ") { "'V${seed}_$it'::character varying" }
    ...
    fun `failed parses are memoised too`() {
        // Deliberately malformed, rather than merely exotic: the shapes that used to fail here — the
        // PostgreSQL enum constraints — now parse, and a test needs an expression that stays rejected.
     ...
     * realistic enum constraint per table: without the memo every generation re-parses all of them,
    ...
    fun `memoisation removes the repeated cost of parsing schema constraints`() {
        val schema = schema(1) { enumCheck(it) }
        val repetitions = 5
