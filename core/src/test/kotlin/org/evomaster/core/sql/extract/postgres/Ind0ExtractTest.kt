...
class Ind0ExtractTest : ExtractTestBasePostgres() {
    ...
    fun testCreateAndExtract() {
        ...
        assertTrue(schema.tables.any { it.id.name == "y" })

        assertEquals(11, schema.tables.first { it.id.name == "x" }.columns.size)
        assertEquals(7, schema.tables.first { it.id.name == "y" }.columns.size)

        assertTrue(schema.tables.first { it.id.name == "x" }.columns.any { it.type.equals("xml", true) })
        assertTrue(schema.tables.first { it.id.name == "y" }.columns.any { it.type.equals("jsonb", true) })

        assertEquals(4, schema.tables.first { it.id.name == "x" }.tableCheckExpressions.size)
        assertEquals(1, schema.tables.first { it.id.name == "y" }.tableCheckExpressions.size)

        // constraints on X table
        assertTrue(schema.tables.first { it.id.name == "x" }.tableCheckExpressions.any { it.sqlCheckExpression.equals("(status = ANY (ARRAY['A'::text, 'B'::text]))") })
        assertTrue(schema.tables.first { it.id.name == "x" }.tableCheckExpressions.any { it.sqlCheckExpression.equals("((status = 'B'::text) = (p_at IS NOT NULL))") })
        assertTrue(schema.tables.first { it.id.name == "x" }.tableCheckExpressions.any { it.sqlCheckExpression.equals("((f_id ~~ 'hi'::text) OR (f_id ~~ '%foo%'::text) OR (f_id ~~ '%foo%x%'::text) OR (f_id ~~ '%bar%'::text) OR (f_id ~~ '%bar%y%'::text) OR (f_id ~~ '%hello%'::text))") })
        assertTrue(schema.tables.first { it.id.name == "x" }.tableCheckExpressions.any { it.sqlCheckExpression.equals("(w_id ~ similar_to_escape('/foo/__/bar/(left|right)/[0-9]{4}-[0-9]{2}-[0-9]{2}(/[0-9]*)?'::text))") })

        // constraints on Y table
        assertTrue(schema.tables.first { it.id.name == "y" }.tableCheckExpressions.any { it.sqlCheckExpression.equals("(status = ANY (ARRAY['A'::text, 'B'::text, 'C'::text, 'D'::text, 'E'::text]))") })
