...
class MonotonicGeneticAlgorithmTest {
    ...
    fun testMonotonicReplacementRule() {
        TestUtils.handleFlaky {
            ...
            // Check monotonicity across recorded generations: best fitness must be non-decreasing
            val best = rec.bestFitnessPerGeneration
            for (k in 1 until best.size) {
                assertTrue(best[k] >= best[k-1])
            }
        }
    }
}
