...
if len(sys.argv) != 10:
    print(
        "Usage:\n<nameOfScript>.py <cluster> <baseSeed> <dir> <minSeed> <maxSeed> <maxActions> <minutesPerRun> <nJobs> <enableProcessMonitor>")
    exit(1)
...
EP = sys.argv[9].lower() in ("yes", "true", "t")
...
def createJobHead(port, sut_name, timeoutMinutes):
    ...
    if CLUSTER:
        ...
        sut_jar_path = os.path.join(CASESTUDY_DIR, sut_name + SUT_POSTFIX)
...
def createOneJob(state, sut_name, seed, weight, algorithm, sampleType,probOfST, problemType, sampleControl):
    code = addJobBody(state.port, sut_name, seed, algorithm, sampleType, probOfST, problemType, sampleControl)
...
def addJobBody(port, sut_name, seed, algorithm, sampleType, probOfST, problemType, sampleControl):
    script = io.StringIO()
    ...
    label = str(algorithm) + "_" + str(sampleType) + "_" + str(problemType) + "_" + str(probOfST) + "_" + str(sampleControl)

    ### Custom for these experiments
    params += " --testSuiteFileName=EM_" + label + "_" + str(seed) + "_Test"
    params += " --algorithm=" + str(algorithm)

    params += " --smartSampling=" + str(sampleType)
    params += " --problemType=" + str(problemType)
    params += " --sampleControl=" + str(sampleControl)
    params += " --enableProcessMonitor=" + str(EP)
    params += " --probOfSmartSampling=" + str(probOfST)
    ...
    for sut in SUTS:
        ...
        for seed in range(MIN_SEED, MAX_SEED + 1):
            ...
            for al in AL:
                ...
                # random.shuffle(ST)
                for sti in range(0, len(ST)):
                    st = ST[sti]
                    for probOfST in PST:
                        if(sti == 1):
                            code = createOneJob(state, sut_name, seed, weight, al, st, probOfST, PT[sti],
                                                SC[0])
                            continue
                        ...
                        for sc in SC:
                            if state.counter == 0:
                                ...
                                code += addJobBody(state.port, sut_name, seed, al, st,probOfST,  PT[sti], sc)
                                state.updateBudget(weight)

                            else:
                                writeWithHeadAndFooter(code, state.port, sut_name, state.getTimeoutMinutes())
                                state.resetTmpForNewRun()
                                code = createOneJob(state, sut_name, seed, weight, al, st, probOfST,  PT[sti], sc)
                            completedForSut += 1
