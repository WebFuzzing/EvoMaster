...
import org.evomaster.client.java.instrumentation.graphs.cfg.ControlFlowGraph;
...
public class DominatorTree<V> extends EvoMasterGraph<DominatorNode<V>, DefaultEdge> {
     ...
     *
     * @param cfg a {@link org.evomaster.client.java.instrumentation.graphs.cfg.ControlFlowGraph} object.
     */
    ...
    private void computeDominatorFrontiers(DominatorNode<V> currentNode) {
        ...
        for (V child : cfg.getChildren(currentNode.node)) {
            ...
            if (y.immediateDominator.dfsOrder != currentNode.dfsOrder) {
        ...
        for (DominatorNode<V> z : getChildren(currentNode)) {
            for (V y : dominatingFrontiers.get(z.node)) {
                if (getDominatorNodeFor(y).immediateDominator.dfsOrder != currentNode.dfsOrder) {
    ...
    private void initialize(DominatorNode<V> currentNode) {
        ...
        nodeCount++;
        currentNode.dfsOrder = nodeCount;
    ...
    private void computeSemiDominators() {
        ...
        // Process nodes in reverse DFS order, starting from the last visited node.
        // We skip node 1 (i >= 2) because node 1 is the root/entry point,
        // which has no dominator and dominates all other nodes.
        for (int i = nodeCount; i >= 2; i--) {
            ...
            for (V current : cfg.getParents(w.node)) {
                ...
                if (u.semiDominator.dfsOrder < w.semiDominator.dfsOrder)
            ...
            while (!w.parent.bucket.isEmpty()) {
                ...
                DominatorNode<V> u = v.eval();
                v.immediateDominator = (u.semiDominator.dfsOrder < v.semiDominator.dfsOrder ? u
                        : w.parent);
            }
        }
    }
