...
import org.evomaster.client.java.instrumentation.graphs.cfg.*;
import java.util.LinkedHashSet;
import java.util.Set;

/**
 * Represents the Control Dependence Graph (CDG) of a method.
 * <p>
 * A CDG captures the control-flow dependencies between basic blocks: an edge from
 * block A to block B indicates that the execution of B depends on a branch decision
 * made in A. 
 * <p>
 * Built from an {@link ActualControlFlowGraph} using dominator frontiers.
 */
public class ControlDependenceGraph extends EvoMasterGraph<BasicBlock, ControlFlowEdge> {
    ...
    private final String dotSubFolderPath;
    ...
    public ControlDependenceGraph(ActualControlFlowGraph cfg) {
        ...
        this.name = methodName + "_CDG";
        this.dotSubFolderPath = toFileString(className) + "/CDG/";
     ...
     * set.
     *
     * @param insBlock a {@link org.evomaster.client.java.instrumentation.graphs.cfg.BasicBlock} object.
    ...
    private void createGraphNodes() {
        ...
        for (BasicBlock b : vertexSet()) {
            if (b.isExitBlock()) {
                boolean removed = graph.removeVertex(b);
                if (!removed) {
    ...
    private void computeControlDependence() {
        ...
        for (BasicBlock b : rcfg.vertexSet()) {
            ...
            for (BasicBlock cd : dt.getDominatingFrontiers(b)) {
                ...
                boolean added = addEdge(cd, b, new ControlFlowEdge(orig));
                if (!added) {
