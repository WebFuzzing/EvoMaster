...
/**
 * The tests every Neo4j {@code run} replacement has to pass, against a real Neo4j started with Testcontainers.
 * The driver exposes the same {@code run} overloads on several receiver types, and each has its own
 * replacement class; a subclass says how to obtain the receiver and which replacement to call, the
 * tests here are the same for all of them.
 */
public abstract class Neo4JRunReplacementTestBase {
    ...
    /**
     * Runs the body with a receiver of the type under test, obtained from a fresh session, and
     * releases it afterwards (closing the session, committing the transaction, ...).
     */
    protected abstract void withRunner(Consumer<Object> body);

    protected abstract Object run(Object runner, String query);

    protected abstract Object run(Object runner, String query, Map<String, Object> parameters);

    protected abstract Object runWithValue(Object runner, String query, Value parameters);

    protected abstract Object runWithRecord(Object runner, String query, Record parameters);

    protected abstract Object run(Object runner, Query query);
    ...
    /** Runs a setup query outside of the replacement, so it is not tracked. */
    private static void setup(String cypher) {
        try (Session session = driver.session()) {
    ...
    private static void createPerson(String name, int age) {
        setup("CREATE (p:Person {name: '" + name + "', age: " + age + "})");
    }

    private static Neo4JRunCommand singleTrackedCommand() {
        ...
        return neo4jCommands.iterator().next();
    }
    ...
    public void testRunWithStringQuery() {
        withRunner(runner -> {
            ...
            Result result = (Result) run(runner, query);
    ...
    public void testRunWithStringQueryAndMapParameters() {
        ...
        withRunner(runner -> {
            ...
            ExecutionTracer.setExecutingInitNeo4J(false);
            Result result = (Result) run(runner, MATCH_PERSON_BY_NAME_QUERY, parameters);
    ...
    public void testRunWithValueParameters() {
        ...
        withRunner(runner -> {
            ...
            ExecutionTracer.setExecutingInitNeo4J(false);
            Result result = (Result) runWithValue(runner, MATCH_PERSON_BY_NAME_QUERY, parameters);
    ...
    public void testRunWithRecordParameters() {
        ...
        Record parameters;
        try (Session session = driver.session()) {
            parameters = session.run("RETURN '" + personName + "' AS " + PARAM_NAME).next();
        }

        withRunner(runner -> {
            ExecutionTracer.setExecutingInitNeo4J(false);
            Result result = (Result) runWithRecord(runner, MATCH_PERSON_BY_NAME_QUERY, parameters);
