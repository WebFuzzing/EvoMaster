...
namespace EvoMaster.Controller.Api {
  public class SutRunDto {
    ...
    public SutRunDto (bool? run, bool? resetState, bool? calculateSqlHeuristics, bool? extractSqlExecutionInfo) {
      ...
      if (calculateSqlHeuristics.HasValue && calculateSqlHeuristics.Value && extractSqlExecutionInfo.HasValue && !extractSqlExecutionInfo.Value)
        throw new ArgumentException ("extractSqlExecutionInfo should be enabled when calculateSqlHeuristics is enabled");
    ...
    public SutRunDto (bool? run, bool? resetState, bool? calculateSqlHeuristics):
      this (run, resetState, calculateSqlHeuristics, calculateSqlHeuristics.HasValue && calculateSqlHeuristics.Value) { }
