...
//        SqlCondition actual = parse(sql, ConstraintDatabaseType.POSTGRES);
    ...
    void testPerformanceIssueInFamilieBaSakWithTimeout() throws Exception{
        ...
        /*
            This used to be asserted to throw: the constraint took minutes and then failed. The cause
            was not the parser library but the dialect transform ahead of it, which ran the ARRAY rule
            to the last "]" in the string — the one in "::text[]" — and handed JSQLParser SQL with
            unbalanced brackets. It now parses, and does so in milliseconds. The timeout is kept as the
            regression guard: if the transform is broken again, this returns to taking minutes.
         */
        SqlCondition parsed = parser.parse(sql, ConstraintDatabaseType.POSTGRES, 3_000);
