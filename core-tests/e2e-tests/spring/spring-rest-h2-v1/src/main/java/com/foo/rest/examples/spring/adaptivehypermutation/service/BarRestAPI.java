...
public class BarRestAPI {
  ...
  public ResponseEntity<Void> createBar(@PathVariable(name = "a") Integer a, @RequestParam(name = "b", required = true) String b, @RequestParam(name = "c") Integer c) {
    // an entity with id bar.id should not exist
    if (barRepository.findById(a).isPresent()) return ResponseEntity.status(400).build();
    if (!b.equalsIgnoreCase("bar")) return ResponseEntity.status(400).build();
  ...
  @RequestMapping(
      ...
      produces = MediaType.APPLICATION_JSON)
  public ResponseEntity<Void> deleteBar(@PathVariable(name = "a") Integer a) {
