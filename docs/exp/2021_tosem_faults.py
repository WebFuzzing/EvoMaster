...
class Config:
     def __init__(self, testSuiteSplitType, expectationsActive, executiveSummary):
            self.testSuiteSplitType = testSuiteSplitType
            self.expectationsActive = expectationsActive
...
def customParameters(seed, config):
    ...
    label = str(config.testSuiteSplitType)

    ### Custom for these experiments
    params += " --testSuiteFileName=EM_" + label + "_" + str(seed) + "_Test"
    params += " --testSuiteSplitType=" + str(config.testSuiteSplitType)
    params += " --expectationsActive=" + str(config.expectationsActive)
    params += " --executiveSummary=" + str(config.executiveSummary)
...
def getConfigs():
   ...
   if CLUSTER:
           CONFIGS.append(Config("CLUSTER", True, True))
