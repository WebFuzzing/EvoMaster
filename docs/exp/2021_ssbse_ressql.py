...
############################################################################
### resource dependency with sql handling experiment settings
############################################################################
#Disable hypermuation
DISABLE_WEIGHT = ParameterSetting("weightBasedMutationRate", [False])
DISABLE_TRACK = ParameterSetting("enableTrackEvaluatedIndividual", [False])
ZERO_IMPACT_MUTATION = ParameterSetting("probOfArchiveMutation", [0.0])
DISABEL_ARCHIVE_MUTATION = ParameterSetting("archiveGeneMutation", ["NONE"])

#Taint
ZERO_TAINT = ParameterSetting("baseTaintAnalysisProbability", [0.0])
ENABLE_TAINT = ParameterSetting("baseTaintAnalysisProbability", [0.9])

#DB
T_H_SOL = ParameterSetting("heuristicsForSQL", [True])
T_G_SQL = ParameterSetting("generateSqlDataWithSearch", [True])

#resourceSampleStrategy
BEST_RES_SMART_SAMPLE = ParameterSetting("resourceSampleStrategy", ["ConArchive"])

#probOfSmartSampling
ZERO_PROB_SS = ParameterSetting("probOfSmartSampling", [0.0])
DEFAULT_PROB_SS = ParameterSetting("probOfSmartSampling", [0.5])
BEST_SMART_SAMPLE_PROB = ParameterSetting("probOfSmartSampling", [1.0])

#doesInvolveDB
NONE_WITH_DB = ParameterSetting("probOfApplySQLActionToCreateResources", [0.0])
ENABLE_WITH_DB = ParameterSetting("probOfApplySQLActionToCreateResources", [0.1, 0.3, 0.5])
BEST_ENABLE_WITH_DB = ParameterSetting("probOfApplySQLActionToCreateResources", [0.5])

#extractSqlExecutionInfo
T_SQL_EXECUTION = ParameterSetting("extractSqlExecutionInfo", [True])
...
## Baseline
RDMIO_BASELINE = Solution("RDMIO_BASELINE", [
    DISABLE_WEIGHT, DISABLE_TRACK, ZERO_IMPACT_MUTATION,
    ENABLE_TAINT, T_H_SOL, T_G_SQL, T_SQL_EXECUTION, NONE_WITH_DB,
    BEST_RES_SMART_SAMPLE, BEST_SMART_SAMPLE_PROB, T_WITH_MATCH, RDMIO_BEST_PROB_DEP_BEST])

RDMIO_SQL = Solution("RDMIO_SQL", [
    DISABLE_WEIGHT, DISABLE_TRACK, ZERO_IMPACT_MUTATION,
