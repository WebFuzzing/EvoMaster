...
def getConfigs():
    ...
    noOnSampling =   [ParameterSetting("taintOnSampling", ["false"])]
    withOnSampling = [ParameterSetting("taintOnSampling", ["true"])]

    noExtra =   [ParameterSetting("extraQueryParam", ["false"]), ParameterSetting("extraHeader", ["false"])]
    withExtra = [ParameterSetting("extraQueryParam", ["true"]), ParameterSetting("extraHeader", ["true"])]

    noTT =   [ParameterSetting("instrumentMR_EXT_0", ["false"])]
    withTT = [ParameterSetting("instrumentMR_EXT_0", ["true"])]

    noJpa =   [ParameterSetting("forceSqlAllColumnInsertion", ["false"]), ParameterSetting("useExtraSqlDbConstraintsProbability", [0.0])]
    withJpa = [ParameterSetting("forceSqlAllColumnInsertion", ["true"]), ParameterSetting("useExtraSqlDbConstraintsProbability", [0.9])]

    CONFIGS.append(Config(noOnSampling+noTT+noExtra+noJpa, "base"))
    CONFIGS.append(Config(withOnSampling+noTT+noExtra+noJpa, "onsampling"))
    CONFIGS.append(Config(noOnSampling+withTT+noExtra+noJpa, "tt"))
    # CONFIGS.append(Config(noOnSampling+noTT+withExtra+noJpa, "extra")) # this made no sense, as extra not check if no TT
    CONFIGS.append(Config(noOnSampling+withTT+withExtra+noJpa, "openapi"))
    CONFIGS.append(Config(noOnSampling+noTT+noExtra+withJpa, "jpa"))
    CONFIGS.append(Config(withOnSampling+withTT+withExtra+withJpa, "all"))
