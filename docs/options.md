...
|`asyncApiCoverageStabilisationMaxMs`| __Int__. White-box AsyncAPI: hard ceiling (ms) on the hybrid wait per fire-and-forget action. If the consumer never settles within this window the search proceeds anyway, accepting whatever coverage was collected so far. *Constraints*: `min=100.0, max=120000.0`. *Default value*: `3000`.|
|`asyncApiCoverageStabilisationPollMs`| __Int__. White-box AsyncAPI: how often (ms) to poll the EM Driver's getTestResults while waiting for a fire-and-forget consumer to settle. Lower values detect completion faster at the cost of more driver round-trips. *Constraints*: `min=10.0, max=10000.0`. *Default value*: `100`.|
|`asyncApiCoverageStabilisationWindowMs`| __Int__. White-box AsyncAPI: time window (ms) the covered-target count must stay unchanged before the hybrid wait declares the SUT's consumer handler done. A larger window is more forgiving of bursty coverage but waits longer. *Constraints*: `min=50.0, max=10000.0`. *Default value*: `300`.|
|`asyncApiFireAndForgetSettleMs`| __Int__. After publishing on a fire-and-forget AsyncAPI operation (one that does not declare a 'reply' channel), how many milliseconds to wait before processing the next action. Gives the SUT's consumer handler time to run before per-action coverage is attributed or the end-of-individual coverage poll fires. Has no effect on 'request'/'reply' operations because the reply-await already serves as a barrier. In white-box AsyncAPI the hybrid coverage-stabilisation strategy supersedes this; the value is then used as the lower bound that applies before the polling loop kicks in. *Constraints*: `min=0.0, max=60000.0`. *Default value*: `200`.|
|`bbAsyncApiUrl`| __String__. When in black-box mode for AsyncAPI, specify the URL of where the AsyncAPI 3.0 schema can be downloaded from. As with bbSwaggerUrl, you can use a 'file://' URL or a plain local file path. *Default value*: `""`.|
|`bbBrokerAuthType`| __Enum__. Black-box AsyncAPI: how EvoMaster authenticates to the broker. Combine with --bbBrokerUsername / --bbBrokerPassword for SASL, or --bbBrokerTruststorePath / --bbBrokerKeystorePath for SSL. *Valid values*: `NONE, SASL_PLAIN, SASL_SCRAM_256, SSL`. *Default value*: `NONE`.|
|`bbBrokerKeystorePassword`| __String__. Black-box AsyncAPI: password for --bbBrokerKeystorePath. *Default value*: `""`.|
|`bbBrokerKeystorePath`| __String__. Black-box AsyncAPI: path to the JKS or PKCS12 keystore for SSL client-cert auth. *Default value*: `""`.|
|`bbBrokerPassword`| __String__. Black-box AsyncAPI: password for SASL/PLAIN or SASL/SCRAM-SHA-256 authentication. *Default value*: `""`.|
|`bbBrokerSaslOverTls`| __Boolean__. Black-box AsyncAPI: when SASL is used, wrap it in TLS (SASL_SSL) instead of plaintext (SASL_PLAINTEXT). Ignored when --bbBrokerAuthType is NONE or SSL. *Default value*: `false`.|
|`bbBrokerTruststorePassword`| __String__. Black-box AsyncAPI: password for --bbBrokerTruststorePath. *Default value*: `""`.|
|`bbBrokerTruststorePath`| __String__. Black-box AsyncAPI: path to the JKS or PKCS12 truststore for SSL-secured brokers. *Default value*: `""`.|
|`bbBrokerUrl`| __String__. When in black-box mode for AsyncAPI, specify the broker bootstrap URL EvoMaster will use to publish requests and observe replies. For Kafka this is the bootstrap-servers value, e.g. 'localhost:9092'. *Default value*: `""`.|
|`bbBrokerUsername`| __String__. Black-box AsyncAPI: username for SASL/PLAIN or SASL/SCRAM-SHA-256 authentication. *Default value*: `""`.|
