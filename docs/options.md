...
|`maximumExistingDataToSampleInDb`| __Int__. Specify a maximum number of existing data in the database to sample when SQL handling is enabled. Note that the negative number means all existing data would be sampled. *Constraints*: `min=-1.0`. *Default value*: `-1`.|
