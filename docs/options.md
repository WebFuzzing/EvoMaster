...
|`jaCoCoLocation`| __String__. Path on filesystem of where JaCoCo jar file is located. Option meaningful only for External Drivers for JVM. If left empty, it is not used. *Constraints*: `regex (.*jacoco.*\.jar)\|(^$)`. *Default value*: `""`.|
|`jaCoCoOutputFile`| __String__. Destination file for JaCoCo. Option meaningful only for External Drivers for JVM. If left empty, it is not used. *Constraints*: `regex (.*jacoco.*\.jar)\|(^$)`. *Default value*: `""`.|
