...
|`epaFile`| __String__. Specify a path to save the EPA. Needs to end in a .dot file. Requires epaCalculation config. *Default value*: `epa.dot`.|
...
|`epaCalculation`| __Boolean__. Creates an EPA for the tested *REST API*. Requires the implementation of the endpoint /enabledEndpoints. *Default value*: `false`.|
...
|`externalServiceIP`| __String__. User provided external service IP. *Constraints*: `regex ^127\.((25[0-5]\|2[0-4][0-9]\|[01]?[0-9][0-9]?)\.){2}([3-9]\|[0-9][0-9]\|[0-2][0-5][0-5]?)$`. *Default value*: `127.0.0.3`.|
...
|`heuristicsForEpa`| __Boolean__. Tracking of EPA edges to improve test generation. Requires epaCalculation config. *Default value*: `false`.|
